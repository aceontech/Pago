    ...
    func controllerForPage(p:Page?) -> UIViewController? {
        if let page = p {
            if let cachedController = self.pageControllers[page.id] {
                ...
                let newController = UIStoryboard(name: page.storyboardResource.storyboard, bundle: NSBundle.mainBundle()).instantiateViewControllerWithIdentifier(page.storyboardResource.id)
